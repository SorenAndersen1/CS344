...
#include <errno.h>
//THIS FILE IS LITERALLY THE EXACT SAME AS ENC_SERVER EXCEPT WITH A FEW DIFFERENT NUMBERS (SAME COMMENTS I COPIED AND PASTED)
...
/*------------------------------------------------- encr -----
         ...
         |  Returns:  None
         *-------------------------------------------------------------------*/
void dcr(char* plaintext, char* key, int messageSize, char* returnval){
	int i = 0;
	int quickMath = 0;
	int holder = 0;
...
// printf("%d %d\n", key[7], plaintext[7] );
	for (i = 0; i < messageSize; i++)
	{
		...
		returnval[i] = (( plaintext[i] - key[i]) + 27) % 27; //add the two nums together than mod 27 to get encrpyt
	...
	char* returnval = malloc(sizeof(char) * *messageSize); //i love C malloc is so fun
		...
		fprintf(stderr, "%s\n", strerror(errno));
				...
				stripMsg(crp, finalPlaintext, key, type); //strip the message for all the individual values
					...
					if(type[0] != '2' && (strlen(finalPlaintext) < 60000)){
						close(connection);
				...
				dcr(finalPlaintext, key, temp, encrString); //do the actual encryption value changing
