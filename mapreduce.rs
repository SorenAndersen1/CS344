...
fn main() {
    ...
    //let num : Arc<Mutex<i32>> = Arc::new(Mutex::new(0));
    // let (tx, rx) = mpsc::channel();
    // let (send, recieve) = mpsc::channel();

    let shared_data = Arc::new(Mutex::new(xs));
    //let anotha_one = Arc::new(Mutex::new(intermediate_sums));
    // let mut result_one = 0;
    // let mut result_two = 0;

    let t1 = thread::spawn({let universal_data = Arc::clone(&shared_data);
        move || {
        let z = universal_data.lock().unwrap();
        return map_data(&z[0]);
    }
    });
        let t2 = thread::spawn({let universal_data = Arc::clone(&shared_data);
        move || {

        let z = universal_data.lock().unwrap();
        return map_data(&z[1]);
        // send.send(map_data(&z[1])).unwrap();
    }
    });
    ...
    let _product : Vec<usize> = Vec::new();
    let _product = partition_data(num_partitions, &v); print_partition_info(&_product);

    let partition_numbers = Arc::new(Mutex::new(_product));
    //let anotha_one = Arc::new(Mutex::new(intermediate_sums));
    // let mut result_one = 0;
    // let mut result_two = 0;
    let mut threads = vec![];
    //println!("Num = {}", num_partitions);
    for x in 0..num_partitions{
       // comms.push(let (tx, rx) = mpsc::channel());
        threads.push(thread::spawn({
            let clone = Arc::clone(&partition_numbers);
             move || {
        let z = clone.lock().unwrap();
        return map_data(&z[x]);
        }
    }));
    }
        let mut not_intermediate_sums : Vec<usize> = Vec::new();


    for t in threads{
        let result = t.join().unwrap();  
        not_intermediate_sums.push(result);
    }
        // Print the vector with the intermediate sums
    println!("Intermediate sums = {:?}", not_intermediate_sums);
        // REDUCE STEP: Process the intermediate result to produce the final result
    let sum = reduce_data(&not_intermediate_sums);
    println!("Sum = {}", sum);
...
fn partition_data(num_partitions: usize, v: &Vec<usize>) -> Vec<Vec<usize>>{
    ...
    for _j in 0..num_partitions {
        ...
        for i in 0..partition_size {
            holder.push(v[num + i]);
        } //according to rubric I dont need to comment this code
        xs.push(holder);
        num = num + partition_size;
    }
        if remainder != 0 {
        for _i in 0..remainder {
                    xs[_i].push(v[v.len() - remainder + _i ]);
                }        
    }
...
//SOURCES OUTSIDE OF CLASS
// https://users.rust-lang.org/t/just-started-learning-rust-how-do-i-modify-a-vector-from-multiple-threads/21046/12
//https://play.rust-lang.org/?gist=2239105222db39d53ccecbae7c34ed81&version=stable&mode=debug&edition=2015
